...
fn quarter<const N: usize>(network: &Network<N>) -> f64 {
    let mut sum = 0;
    for i in 0..UNIT_CIRCLE_STEPS {
        let angle = 2. * PI * i as f64 / UNIT_CIRCLE_STEPS as f64;
        let output = network.evaluate((1., angle));
...
fn two_quarters<const N: usize>(network: &Network<N>) -> f64 {
    let mut sum = 0;
    for i in 0..UNIT_CIRCLE_STEPS {
        let angle = 2. * PI * i as f64 / UNIT_CIRCLE_STEPS as f64;
        let output = network.evaluate((1., angle));
...
fn square<const N: usize>(network: &Network<N>) -> f64 {
